...
bool HelloWorld::init()
{
	...
	do 
	{
        ...
        // [テキスト] スコア
        scoreLabel = CCLabelTTF::create("Score : 0", "arial", 24);
        scoreLabel->setColor(ccc3(100, 200, 300));
        scoreLabel->setPosition(ccp(winSize.width/2, winSize.height - scoreLabel->getContentSize().height));
        this->addChild(scoreLabel,100);
        
        // [テキスト] ハイスコア
        highScoreLabel = CCLabelTTF::create("HighScore : 10", "arial", 24);
        highScoreLabel->setColor(ccc3(100, 300, 300));
        highScoreLabel->setPosition(ccp(winSize.width/2, highScoreLabel->getContentSize().height));
        int highScore = CCUserDefault::sharedUserDefault()->getIntegerForKey("highScore",1);
        CCString* strHighScore = CCString::createWithFormat("HighScore : %d",highScore);
        highScoreLabel->setString(strHighScore->getCString());
        this->addChild(highScoreLabel,100);
